...
macro_rules! replace_self {
    ($desired_self:ident $($toks:tt)*) => {
        $(replace_self_one!($desired_self $toks))*
    };
}

macro_rules! make_dump_test {
    ($self:ident: $($tile_ref:ident)::+ with $fn_to_str:ident { $($func:ident = $($count:literal)? $({ $($count_complex:tt)* })? ;)* }) => {
        impl<
        ...
        > DumpTile for $($tile_ref)::+<D, Ref>
        {
            fn dump(&$self) {
                $({
                    let count = $($count)? $(replace_self!{ $self { $($count_complex)* } })?;
                    for i in 0..count {
                        dump_one_thingy!($self, $fn_to_str, $func, i);
                    }
                })*
            }
        }
...
make_dump_test! {
    self: pannotia::tiles::generic_routing::GenericRoutingRef with hypothetical_autogenerated_fn_to_string {
        rmux = {
